...
gh api repos/luysantanadev/rtk-windows/pulls/<num>/reviews \
  --jq '.[] | {author: .user.login, state: .state, body: .body}'

# 3. Commentaires inline (si CHANGES_REQUESTED)
gh api repos/luysantanadev/rtk-windows/pulls/<num>/comments \
...
```
**PR #<num>** — https://github.com/luysantanadev/rtk-windows.git/pull/<num>
