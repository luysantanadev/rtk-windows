...
| Operation | Frequence | Standard | rtk-windows | Economies |
|-----------|-----------|----------|-----|-----------|
...
```powershell
rtk-windows --version   # Doit afficher "rtk-windows 0.27.x"
rtk-windows gain        # Doit afficher les statistiques d'economies
```

> **Attention** : Un autre projet "rtk" (Rust Type Kit) existe sur crates.io. Si `rtk-windows gain` echoue, vous avez le mauvais package.
...
git status  # Automatiquement reecrit en rtk-windows git status
```

Le hook reecrit de maniere transparente les commandes (ex: `git status` -> `rtk-windows git status`) avant execution.

## Comment ca marche

```
  Sans rtk-windows :                                       Avec rtk-windows :
...
### Fichiers
```powershell
rtk-windows ls .                        # Arbre de repertoires optimise
rtk-windows read file.rs                # Lecture intelligente
rtk-windows read file.rs -l aggressive  # Signatures uniquement
rtk-windows find "*.rs" .               # Resultats compacts
rtk-windows grep "pattern" .            # Resultats groupes par fichier
rtk-windows diff file1 file2            # Diff condense
```

### Git
```powershell
rtk-windows git status                  # Status compact
rtk-windows git log -n 10               # Commits sur une ligne
rtk-windows git diff                    # Diff condense
...
### Tests
```powershell
rtk-windows jest                        # Jest compact
rtk-windows vitest                      # Vitest compact
rtk-windows pytest                      # Tests Python (-90%)
rtk-windows go test                     # Tests Go (-90%)
rtk-windows cargo test                  # Tests Cargo (-90%)
rtk-windows test <cmd>                  # Echecs uniquement (-90%)
```

### Build & Lint
```powershell
rtk-windows lint                        # ESLint groupe par regle
rtk-windows tsc                         # Erreurs TypeScript groupees
rtk-windows cargo build                 # Build Cargo (-80%)
rtk-windows cargo clippy                # Clippy (-80%)
rtk-windows ruff check                  # Linting Python (-80%)
```

### Conteneurs
```powershell
rtk-windows docker ps                   # Liste compacte
rtk-windows docker logs <container>     # Logs dedupliques
rtk-windows kubectl pods                # Pods compacts
```

### Analytics
```powershell
rtk-windows gain                        # Statistiques d'economies
rtk-windows gain --graph                # Graphique ASCII (30 jours)
rtk-windows discover                    # Trouver les economies manquees
```
